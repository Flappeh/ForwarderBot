...
class AccountCRUDRepository(BaseCRUDRepository):
    async def create_account(self, account_create: AccountInCreate, role_id: int) -> Account:
        new_account = Account(
            username=account_create.username, email=account_create.email, is_logged_in=True, role_id=role_id
        )
    ...
    async def read_user_by_password_authentication(self, account_login: AccountInLogin) -> Account:
        ...
        # Test with email
        if not db_account:
            stmt = sqlalchemy.select(Account).where(Account.email == account_login.username)
            query = await self.async_session.execute(statement=stmt)
            db_account = query.scalar()
    ...
    async def update_account_by_id(self, id: int, account_update: AccountInUpdate, role_id: int = None) -> Account:
    ...
    async def delete_account_profile(self, id: int) -> str:
        select_stmt = sqlalchemy.select(Account).where(Account.id == id)
        query = await self.async_session.execute(statement=select_stmt)
    ...
    async def user_delete_account_by_id(self, id: int) -> str:
        select_stmt = sqlalchemy.select(Account).where(Account.id == id)
        query = await self.async_session.execute(statement=select_stmt)
        delete_account = query.scalar()
        ...
        stmt = sqlalchemy.update(table=Account).where(Account.id == delete_account.id).values(delete_flag = True)
